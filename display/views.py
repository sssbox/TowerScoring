...
from match.models import ScoringSystem, Match
...
def get_timer_dict():
    ...
    try:
        red_timer = (match.red_center_active_start + datetime.timedelta(seconds=31)) - datetime.datetime.now()
        if red_timer.days < 0:
            red_timer = '00'
        else:
    ...
    try:
        blue_timer = (match.blue_center_active_start + datetime.timedelta(seconds=31)) - datetime.datetime.now()
